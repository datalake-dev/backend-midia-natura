...
try:
    ...
    **Regra de Ouro:** Use **EXCLUSIVAMENTE** os nomes de colunas fornecidos no esquema acima. Não infira ou utilize qualquer outro nome de coluna ou pseudo-coluna (como `_PARTITIONTIME`).
    ...
    **Regras de Geração:**
    ...
    # Este bloco captura erros na inicialização
    print("!!!!!!!!!! ERRO NA INICIALIZAÇÃO DO SERVIÇO !!!!!!!!!!")
...
def gemini_chat(request):
    try:
        # (O resto do código continua como na última versão funcional que discutimos)
        ...
        if results.empty:
            final_answer = "Não encontrei dados para a sua pergunta. Por favor, tente refinar sua busca."
            ...
            analysis_prompt = (f"Com base nos seguintes dados extraídos do BigQuery em formato CSV:\n\n{data_as_string}\n\nResponda à pergunta original do usuário de forma clara, analítica e proativa: '{user_question}'.\n\n---FIM DA ANÁLISE---\nAgora, adicione o separador `###SUGESTÕES###` e, em uma nova linha, sugira 2 ou 3 perguntas de acompanhamento. Não adicione nenhum outro texto após as sugestões.")
        ...
        print(traceback.format_exc())
        final_answer = "Desculpe, não consegui processar sua solicitação
