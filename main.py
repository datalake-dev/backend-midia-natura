...
1.  **Análise para Otimização:** Se a pergunta for sobre otimização, rentabilização ou comparação (ex: 'como melhorar', 'qual o melhor', 'compare'), não retorne apenas um número. Em vez disso, retorne a métrica principal AGRUPADA pelas dimensões mais relevantes para a análise (como `Segmentacao`, `LinhaCriativa`, `Formato`, `Plataforma`). Por exemplo, para "como rentabilizar o CPM?", gere um SQL como `SELECT Segmentacao, Formato, (SUM(Cost) / SUM(Impressions)) * 1000 AS CPM FROM tabela GROUP BY Segmentacao, Formato ORDER BY CPM ASC`.
--- FIM DA ATUALIZAÇÃO ---
2.  Responda APENAS com o código SQL. Não inclua explicações ou markdown.
3.  Use a cláusula `WHERE` sempre que possível para filtrar os dados.
4.  **Filtros Flexíveis e Sinônimos:** Para colunas de texto, use `LOWER(nome_da_coluna) LIKE '%termo%'`. Aplique isso para as seguintes colunas e seus sinônimos:
    * `Campanha`: para buscas por nome de campanha.
    * `Plataforma`: para buscas por plataforma (ex: Meta, Google, TikTok).
    * `Segmentacao`: quando o usuário mencionar 'audiência', 'segmentação', 'segmentacao', ou 'audiencia'.
    * `Canal`: para buscas por canal.
    * `ObjetivoDeMidia`: quando o usuário mencionar 'objetivo de mídia'.
    * `ObjetivodeComunicacao`: quando o usuário mencionar 'objetivo de comunicação'.
    * `PrecisionMkt`: quando o usuário mencionar 'precision mkt', 'precision', 'precision marketing', ou 'pm'.
5.  A data de hoje é {pd.Timestamp.now().strftime('%Y-%m-%d')}. Use isso como referência para filtros de data.
